...
class MFModel(object):
    def __init__(self, F,
        ...
        self._factors = F
        self._users_nums = users_num
        self._items_nums = items_num
    ...
    def initialize(self, loc: float = 0, scale: float = 0.1):
        ...
        self._user_bias = np.zeros(self._users_nums)
        self._item_bias = np.zeros(self._items_nums)
        self._user_factors = \
            np.random.normal(loc=loc, scale=scale, size=(self._users_nums, self._factors))
        self._user_factors = np.around(self._user_factors,decimals = 6)
        self._item_factors = \
            np.random.normal(loc=loc, scale=scale, size=(self._items_nums, self._factors))
        self._item_factors = np.around(self._item_factors,decimals = 6)

    def indexed_predict(self, user, item):
        score_list = []
        for u,i in zip(user,item):
            score = self._global_bias + self._user_bias[u] + self._item_bias[i] \
    ...
    def train_step(self, batch, **kwargs):
        ...
        reg = self._reg
        count = 0
        for user, item, rating in tqdm.tqdm(batch):
            ...
            if count% int(len(batch)/4) ==0 or count == len(batch) :
                self._user_factors = np.around(self._user_factors,decimals = 6)
                self._item_factors = np.around(self._item_factors,decimals = 6)
                self._user_bias = np.around(self._user_bias,decimals = 6)
                self._global_bias = np.around(self._global_bias,decimals = 6)
                self._item_bias = np.around(self._item_bias,decimals = 6)
